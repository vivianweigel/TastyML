...
recipe_lookup <- df %>%
  ...
  distinct(recipe_code, .keep_all = TRUE)  # <- ensures only 1 row per recipe_code
...
predict_one_recipe <- function(rating_matrix, user_id, recipe_code, k = 5, 
  ...
  if (method == "user") {
    ...
    # Run item-item collaborative filtering (function to be defined separately)
