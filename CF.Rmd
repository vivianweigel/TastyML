...
```{r}
user_user_df <- function(df, user, sim_metric = "Cosine", k = 2, 
...
```{r}
item_item_df <- function(df, user, sim_metric = "Cosine", k = 2, 
                         recipe_lookup_df = NULL) {
...
predicted_ratings <- user_user_df(rating_matrix, user = test_user, 
...
predicted_ratings <- item_item_df(rating_matrix, user = test_user, 
                                  sim_metric = "Cosine", k = 5, 
                                  recipe_lookup_df = recipe_lookup)
...
```{r}
# pick a user with a good number of ratings
user_rating_counts <- rowSums(!is.na(rating_matrix))
test_user <- names(sort(user_rating_counts, decreasing = TRUE))[2]  # most active user
...
held_out_items <- sample(rated_items, 5)
...
pred_user_user <- user_user_df(test_matrix, user = test_user, 
                               sim_metric = "Cosine", k = 5, 
                               recipe_lookup_df = recipe_lookup)
pred_item_item <- item_item_df(test_matrix, user = test_user, 
                               sim_metric = "Cosine", k = 5,
                               recipe_lookup_df = recipe_lookup)
...
# add actual held-out ratings
comparison$actual <- true_ratings[comparison$recipe_code]

# reorder columns for readability
comparison <- comparison %>%
  select(recipe_code, recipe_name, user_pred, item_pred, actual)
