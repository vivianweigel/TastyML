...
### testing CF models by hiding ratings and analysis
```{r}
library(ggplot2)
library(dplyr)
...
for (user in rownames(test_rating_matrix)) {
  result <- evaluate_user_cf(user, rating_matrix, seed = 42, k = 5, 
...
# MSE
paste("MSE (user-user):", mean((results_df$user_pred - results_df$actual)^2, na.rm = TRUE))
paste("MSE (item-item):", mean((results_df$item_pred - results_df$actual)^2, na.rm = TRUE))
...
per_user_mse <- ggplot(user_level_perf, aes(x = mse_user, y = mse_item, color = num_ratings)) +
  geom_point() +
...
mean(user_level_perf$mse_user)
mean(user_level_perf$mse_item)
...
ggplot(user_level_perf, aes(x = num_ratings, y = mse_user - mse_item)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Which CF Model Performs Better by User Rating Count",
       y = "RMSE Difference (User - Item)", x = "# of Ratings")
```
User-user CF outperforms item-item CF for the majority of users, especially in terms of MSE. However, item-item occasionally performs better for certain users — a hybrid might be useful for edge cases.
